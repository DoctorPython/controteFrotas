...
const createDefaultRules = (): GeofenceRule[] => [
...
const mergeRulesWithDefaults = (rules?: GeofenceRule[]): GeofenceRule[] => {
  const defaults = createDefaultRules();
  if (!rules || rules.length === 0) return defaults;

  const merged = defaults.map((defaultRule) => {
    const existing = rules.find(rule => rule.type === defaultRule.type);
    return existing ? { ...defaultRule, ...existing } : defaultRule;
  });

  const extraRules = rules.filter(
    rule => !defaults.some(defaultRule => defaultRule.type === rule.type)
  );

  return [...merged, ...extraRules];
};

const createEmptyForm = (): GeofenceFormState => ({
  name: "",
  description: "",
  type: "circle",
  active: true,
...
const buildFormFromGeofence = (geofence: Geofence): GeofenceFormState => ({
  name: geofence.name,
  ...
  rules: mergeRulesWithDefaults(geofence.rules),
...
export default function GeofencesPage() {
  ...
  const [formData, setFormData] = useState<GeofenceFormState>(() => createEmptyForm());
  ...
  const createMutation = useMutation({
    mutationFn: async (data: GeofenceFormState) => {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: GeofenceFormState }) => {
      return apiRequest("PATCH", `/api/geofences/${id}`, data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/geofences"] });
      toast({ title: "Geofence atualizada", description: "As alterações foram salvas com sucesso." });
      closeForm();
    },
    onError: () => {
  ...
  const handleOpenEdit = (geofence: Geofence) => {
    setFormMode("edit");
    setEditingGeofence(geofence);
    setFormData(buildFormFromGeofence(geofence));
    setIsFormOpen(true);
  };
  ...
  const handleSubmit = () => {
    ...
    const isEditing = formMode === "edit" && editingGeofence;
    if (isEditing) {
  ...
  return (
    <div className="h-full flex flex-col" data-testid="geofences-page">
      <div className="p-6 border-b border-border">
        <h1 className="text-2xl font-bold mb-1" data-testid="text-page-title">Geofences</h1>
        <p className="text-muted-foreground">Crie e gerencie áreas virtuais para monitoramento</p>
      </div>

      <div className="flex-1 flex min-h-0">
        <div className="w-[400px] flex-shrink-0 border-r border-border bg-sidebar flex flex-col">
          <div className="p-4 border-b border-sidebar-border flex items-center justify-between">
            <h2 className="font-semibold text-lg">Lista de Geofences</h2>
          <Button onClick={handleOpenCreate} className="gap-2" data-testid="button-create-geofence">
      ...
      <Dialog open={isFormOpen} onOpenChange={handleDialogOpenChange}>
        <DialogContent className="max-w-4xl h-[90vh] flex flex-col">
          <DialogHeader>
            <DialogTitle>{formMode === "edit" ? "Editar Geofence" : "Criar Nova Geofence"}</DialogTitle>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSubmit} disabled={isSaving} data-testid="button-save-geofence">
              {isSaving ? "Salvando..." : formMode === "edit" ? "Salvar alterações" : "Salvar Geofence"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
